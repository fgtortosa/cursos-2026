---
title: "Respuestas — Sesión 6: Introducción a .NET"
description: "Solucionario razonado del test de 22 preguntas de la Sesión 6."
outline: [2, 2]
search: false
---

# Respuestas — Test Sesión 6: Introducción a .NET
...
9. **b)** Actúa como caso por defecto, capturando cualquier valor no contemplado. El `_` (_discard pattern_) en un `switch` expression funciona como el `default:` de un switch clásico.
...
15. **b)** `true`. Las _raw string literals_ (`"""..."""`) de C# 11+ permiten escribir strings multilínea sin escapar. El string resultante contiene `NOMBRE_ES`, por lo que `Contains` devuelve `true`.
