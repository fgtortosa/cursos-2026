---
url: /curso-normalizacion/02-dotnet/sesiones/sesion-07-dtos-apis.md
description: >-
  DTOs, controladores API REST, verbos HTTP, status codes, documentación Scalar
  y prueba sin BD desde Chrome
---
...
* Propiedades en **PascalCase** en C# → se mapean automáticamente a **SNAKE\_CASE** en Oracle (`FechaModificacion` → `FECHA_MODIFICACION`, `IdTipoRecurso` → `ID_TIPO_RECURSO`).
* Los `bool` de C# se mapean a `VARCHAR2(1)` con valores `'S'` / `'N'` en Oracle.
* Usa `[Columna("NOMBRE_REAL")]` solo si la columna no sigue la convención SNAKE\_CASE.
  :::
