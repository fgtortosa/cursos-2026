---
title: "Sesión 3 — Tablas, vistas y paquetes"
...
Ruta mínima en 90 min: _Antes de empezar → Objetivos → Entidad de ejemplo → Paso 1 → Paso 2 → Paso 3 → Gestión de errores → Práctica guiada (puntos 1–5 y 7) → Ejercicio entregable → Resumen_. Lo demás es ampliación.
:::

## Antes de empezar · `IMPRESCINDIBLE` {#antes-de-empezar}

Esta sesión continúa directamente desde la [Sesión 1 — Fundamentos Oracle](../sesion-01-docencia-fundamentos-oracle/). Antes de ejecutar scripts, confirma que tienes claro el punto de partida:
...
::: tip CONTINUIDAD
Si no has completado el [ejercicio de la sesión 1](../sesion-02-ejercicio-fundamentos/) ni el checklist de continuidad, vuelve primero allí. En esta sesión trabajamos con dos conexiones (`CURSONORMADM` y `CURSONORMWEB`) que deben estar listas.
:::
...
(
    -- IDENTITY: Oracle genera el valor automáticamente al insertar.
    -- BY DEFAULT ON NULL: solo genera si la columna llega a NULL (permite forzar un ID en migraciones).
    -- CONSTRAINT ... NOT NULL: nombra el CHECK de obligatoriedad; patrón CK_<TABLA>_<COLUMNA>_NN.
    ID_TIPO_RECURSO NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY
        CONSTRAINT CK_TRES_TIPO_RECURSO_ID_NN        NOT NULL,

    -- VARCHAR2(100): texto de hasta 100 bytes. NOT NULL nombrado para tener un mensaje de error legible.
    CODIGO          VARCHAR2(100)
        CONSTRAINT CK_TRES_TIPO_RECURSO_CODIGO_NN    NOT NULL,

    -- Tres columnas de nombre, una por idioma. Todas obligatorias.
    ...
    -- PRIMARY KEY: identifica unívocamente cada fila. Implica NOT NULL e índice único.
    -- Patrón de nombre: PK_<TABLA>.
    CONSTRAINT PK_TRES_TIPO_RECURSO        PRIMARY KEY (ID_TIPO_RECURSO),

    -- UNIQUE: garantiza que no puede haber dos filas con el mismo CODIGO.
    -- No es la PK (el ID técnico lo es), pero sí la identidad funcional del catálogo.
    -- Patrón de nombre: UK_<TABLA>_<COLUMNA>.
...
| Constraints `CK_TRES_TIPO_RECURSO_*_NN` | Cada `NOT NULL` lleva un nombre semántico siguiendo el patrón `CK_<TABLA>_<COLUMNA>_NN`. Es una de las **seis variantes de CHECK** vistas en la sesión 1. |
| Sin `ACTIVO`                            | No hay borrado lógico en este catálogo; `ELIMINAR` borra físicamente si no hay recursos asociados.                                                        |

::: tip BUENA PRÁCTICA
Nombramos cada constraint de forma semántica (`CK_<TABLA>_<COLUMNA>_NN`) para que los mensajes de error de Oracle (`ORA-02290`, `ORA-01400`) identifiquen inmediatamente qué columna y qué tabla han fallado, sin tener que consultar el diccionario de datos.
:::

### La tabla hija: `TRES_RECURSO`

`TRES_RECURSO` representa cada recurso reservable concreto. Referencia a `TRES_TIPO_RECURSO` mediante FK y añade flags de estado (`ACTIVO`, `VISIBLE`) y campos de configuración de reserva (`GRANULIDAD`, `DURACION`).

| Columna                 | Tipo                      | Papel                                        |
| ----------------------- | ------------------------- | -------------------------------------------- |
| `ID_RECURSO`            | `NUMBER IDENTITY`         | Clave primaria técnica                       |
| `NOMBRE_ES/CA/EN`       | `VARCHAR2(200)`           | Nombre del recurso en los tres idiomas       |
| `DESCRIPCION_ES/CA/EN`  | `CLOB`                    | Descripción larga (opcional)                 |
| `FECHA_MODIFICACION`    | `DATE DEFAULT SYSDATE`    | Auditoría básica; obligatoria                |
| `GRANULIDAD`            | `NUMBER`                  | Minutos mínimos de reserva (múltiplo)        |
| `DURACION`              | `NUMBER`                  | Duración por defecto de la reserva (minutos) |
| `ID_TIPO_RECURSO`       | `NUMBER` FK               | Tipo al que pertenece el recurso             |
| `ACTIVO`                | `VARCHAR2(1) DEFAULT 'S'` | Borrado lógico: `'S'` activo, `'N'` inactivo |
| `VISIBLE`               | `VARCHAR2(1) DEFAULT 'S'` | Aparece en el buscador público               |
| `ATIENDE_MISMA_PERSONA` | `VARCHAR2(1) DEFAULT 'N'` | Requiere asignar una persona responsable     |

```sql
CREATE TABLE CURSONORMADM.TRES_RECURSO
(
    -- IDENTITY + NOT NULL nombrado. Mismo patrón que TRES_TIPO_RECURSO.
    ID_RECURSO            NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY
        CONSTRAINT CK_TRES_RECURSO_ID_NN           NOT NULL,

    -- Nombres del recurso en los tres idiomas. Todos obligatorios (NOT NULL nombrado).
    ...
    -- CLOB: texto de longitud ilimitada. Sin NOT NULL porque la descripción es opcional.
    DESCRIPCION_ES        CLOB,
    DESCRIPCION_CA        CLOB,
    DESCRIPCION_EN        CLOB,

    -- DEFAULT SYSDATE: Oracle rellena la fecha actual si el cliente no la informa.
    -- NOT NULL nombrado: garantiza que siempre quede registrada la fecha de modificación.
    FECHA_MODIFICACION    DATE DEFAULT SYSDATE
        CONSTRAINT CK_TRES_RECURSO_FECHA_MOD_NN    NOT NULL,

    -- GRANULIDAD: unidad mínima de reserva, en minutos. Define el "paso" del calendario.
    --   Ejemplo: GRANULIDAD = 30 → las reservas solo pueden empezar a :00 o :30.
    --   NULL significa que el recurso no tiene restricción de granularidad.
    GRANULIDAD            NUMBER,

    -- DURACION: duración por defecto de una reserva, en minutos.
    --   La interfaz la preselecciona al crear una nueva reserva, pero el usuario puede cambiarla.
    --   Ejemplo: DURACION = 60 → al abrir el formulario de reserva aparece 1 hora preseleccionada.
    --   NULL significa que no hay duración predefinida.
    DURACION              NUMBER,

    -- FK declarada aquí como NUMBER; la restricción referencial se define abajo.
    ID_TIPO_RECURSO       NUMBER,

    -- DEFAULT 'S' + NOT NULL nombrado: el flag nunca puede quedar indefinido.
    -- El CHECK de dominio (IN 'S','N') se añade como constraint de tabla al final.
    -- Sin el NOT NULL, un NULL pasaría el CHECK IN sin violar la restricción en Oracle.
    ...
    -- PRIMARY KEY de la tabla. Patrón PK_<TABLA>.
    CONSTRAINT PK_TRES_RECURSO
        PRIMARY KEY (ID_RECURSO),

    -- FOREIGN KEY: ID_TIPO_RECURSO debe existir en TRES_TIPO_RECURSO.
    -- Sin ON DELETE CASCADE: Oracle impide borrar un tipo si tiene recursos asociados (ORA-02292).
    -- Patrón: FK_<TABLA_HIJA>_<TABLA_PADRE>.
    ...
    -- CHECK de dominio cerrado: solo admite 'S' o 'N'.
    -- Complementa al NOT NULL de columna: juntos cierran completamente el dominio del flag.
    -- Patrón: CK_<TABLA>_<COLUMNA> (sin _NN porque no es una restricción de nulidad).
    CONSTRAINT CK_TRES_RECURSO_ACTIVO
        CHECK (ACTIVO  IN ('S', 'N')),
    CONSTRAINT CK_TRES_RECURSO_VISIBLE
        CHECK (VISIBLE IN ('S', 'N'))
);
```

### Qué decisiones adicionales enseña `TRES_RECURSO`

| Elemento                              | Lectura técnica                                                                                                       |
| ------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `FK_TRES_RECURSO_TIPO_RECURSO`        | Garantiza que todo recurso apunta a un tipo existente. Sin esta FK, podrían quedar huérfanos.                         |
| `ACTIVO` y `VISIBLE` con `CHECK IN`   | Dos flags independientes: uno controla si el recurso existe operativamente; el otro si aparece en la interfaz.        |
| `CK_TRES_RECURSO_ACTIVO_NN` + dominio | El `NOT NULL` y el `CHECK IN` van juntos: sin el `NOT NULL`, un `NULL` pasaría el `IN ('S','N')` en Oracle.           |
| `CLOB` para descripciones             | Textos largos no acotables. No llevan `NOT NULL` porque la descripción es opcional.                                   |
| `FECHA_MODIFICACION DEFAULT SYSDATE`  | Auditoría mínima: Oracle rellena la fecha si el cliente no la informa. El `NOT NULL` obliga a que siempre haya valor. |
...
Hasta aquí hemos tratado `VRES_TIPO_RECURSO` como un contrato de **lectura**. En Oracle, sin embargo, una vista puede aceptar `UPDATE`, `INSERT` y `DELETE` directos cuando cumple ciertas condiciones: las llamamos _vistas actualizables_ o _key-preserved views_.
...
| ------------------------------ | ------------------------------------------------ | ----------------------------------------------------------------------------------------------------- |
| Origen                         | Una sola tabla, sin `JOIN`.                      | Vista con `JOIN` (excepto columnas de la tabla _key-preserved_).                                      |
| Proyección                     | Columnas de la tabla, sin expresiones complejas. | Columnas calculadas (`UPPER`, concatenaciones, `CASE`...).                                            |
| Agregación                     | Sin `GROUP BY`, `DISTINCT`, `ROWNUM`, `UNION`.   | Cualquiera de ellas hace la vista _no actualizable_.                                                  |
| Solución si no es actualizable | —                                                | Crear un _trigger_ `INSTEAD OF UPDATE/INSERT/DELETE` que traduzca la operación a la(s) tabla(s) base. |

::: warning IMPORTANTE
`VRES_RECURSO` (que veremos después) tiene `LEFT JOIN` con `TRES_TIPO_RECURSO`. Las columnas de `TRES_RECURSO` siguen siendo actualizables (es la _key-preserved table_), pero los nombres del tipo de recurso (`NOMBRE_TIPO_RECURSO_*`) **no** se pueden actualizar por la vista: Oracle responde `ORA-01779: cannot modify a column which maps to a non key-preserved table`. En ese caso, el patrón es un _INSTEAD OF trigger_ — fuera del alcance de esta sesión.
:::
...
#### Por qué declaramos `T_CURSOR` en la especificación

Un **REF CURSOR** es un tipo que representa un puntero a un resultado de consulta abierto en la base de datos. El procedimiento abre el cursor (ejecuta el `SELECT`) y se lo entrega a .NET; .NET lo recorre fila a fila sin que el package tenga que materializar toda la lista en memoria.

##### Las tres opciones para devolver filas: `CURSOR`, `SYS_REFCURSOR` y `T_CURSOR`

| Opción                        | ¿Válida como parámetro `OUT`?                       | Requiere declarar TYPE | Convención UA |
| ----------------------------- | --------------------------------------------------- | ---------------------- | ------------- |
| `CURSOR`                      | **No** — es una palabra clave, no un tipo           | —                      | —             |
| `SYS_REFCURSOR`               | **Sí** — tipo predefinido por Oracle desde 9i       | No                     | No preferida  |
| `TYPE T_CURSOR IS REF CURSOR` | **Sí** — tipo débil definido en la spec del package | Sí, en la SPEC         | **Preferida** |

`CURSOR` en PL/SQL es una palabra clave para declarar cursores explícitos con nombre y `SELECT` fijo; no se puede pasar como parámetro. Para eso existe `REF CURSOR`, que sí es un tipo.

`SYS_REFCURSOR` es el alias predefinido por Oracle para un REF CURSOR débil (sin `RETURN`). Funciona igual que `T_CURSOR` y el resultado en .NET es idéntico. La razón de preferir `T_CURSOR` es de **convención de equipo**: el tipo queda dentro del namespace del package (`PKG_RES_TIPO_RECURSO.T_CURSOR`), lo que deja claro de dónde viene si se reutiliza en otro contexto.

Definimos `TYPE T_CURSOR IS REF CURSOR` en la **especificación** (no en el body) por dos razones:

1. **Visibilidad pública**: los procedimientos `OBTENER_TODOS` y `OBTENER_POR_ID` tienen `P_CURSOR OUT T_CURSOR` en su firma. Para que esa firma sea visible desde fuera del package, el tipo también debe estar en la spec. Si lo pusiéramos en el body, Oracle no lo reconocería al compilar la llamada desde .NET.
2. **Tipo compartido**: la spec actúa como contrato. Cualquier paquete o código externo que quiera declarar una variable del mismo tipo puede usar `PKG_RES_TIPO_RECURSO.T_CURSOR` sin necesidad de redefinirlo.

`P_CURSOR OUT T_CURSOR` significa:

- `P_CURSOR` — nombre del parámetro.
- `OUT` — el procedimiento **escribe** en él; el llamante lo recibe como resultado (no lo envía).
- `T_CURSOR` — el tipo, que es un REF CURSOR débil (sin `RETURN`); puede apuntar a cualquier `SELECT`.

En .NET, `ClaseOracleBD3` recoge ese parámetro `OUT` como un `OracleDataReader` y lo itera con `Read()`.
...
CREATE OR REPLACE PACKAGE CURSONORMADM.PKG_RES_TIPO_RECURSO AS
  ...
  -- TYPE en la SPEC (no en el BODY) para que sea visible desde fuera del package.
  --
  -- Opciones posibles para devolver filas:
  --   · CURSOR          → palabra clave PL/SQL, NO es un tipo; no se puede usar como OUT.
  --   · SYS_REFCURSOR   → tipo predefinido por Oracle (equivalente a weak REF CURSOR).
  --                        Válido, pero no deja claro a qué package pertenece.
  --   · T_CURSOR        → convención UA: mismo comportamiento que SYS_REFCURSOR,
  --                        pero nombrado dentro del namespace del package.
  --
  -- REF CURSOR débil (sin RETURN): puede apuntar a cualquier SELECT, sin fijar columnas.
  -- El procedimiento hace OPEN P_CURSOR FOR SELECT ...; .NET lo recibe como OracleDataReader.
  ...
  PROCEDURE CREAR(
    ...
    P_ID_TIPO_RECURSO OUT CURSONORMADM.TRES_TIPO_RECURSO.ID_TIPO_RECURSO%TYPE,  -- nuevo ID generado por IDENTITY
  ...
  -- Listado completo.
  -- P_CURSOR OUT T_CURSOR: el procedimiento abre el SELECT y pasa el cursor abierto a .NET.
  -- .NET lo recibe como OracleDataReader y llama a Read() para iterar las filas.
  PROCEDURE OBTENER_TODOS(P_CURSOR OUT T_CURSOR);

  -- Lectura por ID. Devuelve un cursor con 0 o 1 filas.
  -- Si el ID no existe, el cursor se abre igualmente pero no devuelve ninguna fila (Read() = false).
  PROCEDURE OBTENER_POR_ID(
    P_ID_TIPO_RECURSO IN  CURSONORMADM.TRES_TIPO_RECURSO.ID_TIPO_RECURSO%TYPE,
    P_CURSOR          OUT T_CURSOR  -- cursor abierto: 0 filas si no existe, 1 fila si existe
  );
...
BEGIN
  ...
  IF P_VALOR IS NULL OR P_VALOR <= 0 THEN
    ...
    -- Formato '# mensaje #': .NET lo detecta como EnumBDException.Usuario (visible al usuario).
...
--   · OUT     → el procedimiento escribe en él; la app lo recibe, no lo envía.
--   · T_CURSOR → REF CURSOR débil definido en la SPEC; equivalente a SYS_REFCURSOR
--               pero dentro del namespace del package (convención UA).
--   · En .NET → ClaseOracleBD3 lo recoge como OracleDataReader; se itera con Read().
PROCEDURE OBTENER_TODOS(P_CURSOR OUT T_CURSOR) AS
BEGIN
  -- OPEN P_CURSOR FOR asocia el parámetro OUT a la consulta.
  -- La consulta NO se ejecuta aquí: se ejecuta cuando .NET empieza a leer del cursor.
  -- Alternativa válida: OPEN P_CURSOR FOR SELECT ... (usando SYS_REFCURSOR daría el mismo resultado).
  OPEN P_CURSOR FOR
...
-- Mismo patrón con filtro por PK. El cursor puede devolver 0 o 1 filas.
-- La app detecta "no encontrado" cuando Read() devuelve false en el primer intento.
PROCEDURE OBTENER_POR_ID(
  P_ID_TIPO_RECURSO IN  CURSONORMADM.TRES_TIPO_RECURSO.ID_TIPO_RECURSO%TYPE,
  P_CURSOR          OUT T_CURSOR  -- OUT: el procedimiento entrega el cursor abierto
) AS
BEGIN
...
- **Menos código que escribir y mantener.** Una vista bien diseñada y un `SELECT` en el servicio son suficientes para el 80% de los listados.1
...
> **Detalle técnico:** ciclo de vida de un `REF CURSOR`, lectura consistente, `OPEN_CURSORS`, _strong vs weak_ y consumo desde .NET están en [Para profundizar — REF CURSOR](#detalle-ref-cursor) al final de la página.
...
Sin la comprobación de `SQL%ROWCOUNT`, una llamada a `ACTUALIZAR` con un ID inexistente parecería correcta: Oracle no devolvería error porque el `WHERE` simplemente no afecta a ninguna fila. El `RAISE_APPLICATION_ERROR(-20702, '# El tipo de recurso no existe. #')` lo convierte en error explícito; el `EXCEPTION WHEN OTHERS` lo recoge y lo deja en `P_CODIGO_ERROR = -20702`, `P_MENSAJE_ERROR = 'ORA-20702: # El tipo de recurso no existe. #'`. El extractor de .NET detecta el formato `#...#` y muestra al usuario **"El tipo de recurso no existe."** con `TipoExcepcion = EnumBDException.Usuario`; sin los delimitadores, el mismo mensaje se trataría como error de sistema y no llegaría al usuario.
...
BEGIN
  ...
  IF V_TOTAL_RECURSOS > 0 THEN
    -- Formato '# mensaje #': visible al usuario (EnumBDException.Usuario).
...
### El formato `# ... #`: mensajes visibles al usuario {#formato-mensajes}

Oracle no conoce el idioma del usuario ni sabe si un error es funcional o técnico. La librería `ClaseOracleBD` de la UA sí: al recibir un error Oracle, extrae el texto y decide cómo tratarlo.

La regla es simple: **si el mensaje del `RAISE_APPLICATION_ERROR` va delimitado entre `#` y `#`, es un mensaje funcional visible al usuario**. Sin los delimitadores, se trata como un error de sistema.

| Formato del `RAISE_APPLICATION_ERROR`                  | `TipoExcepcion`           | ¿El usuario ve el texto?      |
| ------------------------------------------------------ | ------------------------- | ----------------------------- |
| `'Error interno inesperado.'`                          | `EnumBDException.Sistema` | No — solo llega a los logs    |
| `'# El tipo de recurso no existe. #'`                  | `EnumBDException.Usuario` | Sí — texto literal tal cual   |
| `'# Resources.Errores.TipoNoExiste #'`                 | `EnumBDException.Usuario` | Sí — traducido desde `.resx`  |
| `'# Resources.Errores.TipoNoExiste\|Param1\|Param2 #'` | `EnumBDException.Usuario` | Sí — traducido con parámetros |

En este package usamos **mensajes literales** (`# texto fijo #`) porque la entidad es sencilla y no necesita traducción por idioma. En packages más complejos podríamos usar claves de recursos para internacionalizar el mensaje.

::: warning IMPORTANTE
Un `RAISE_APPLICATION_ERROR` **sin** `#...#` genera `EnumBDException.Sistema`: el texto llega a los logs pero **nunca** se muestra al usuario. Es el formato correcto para errores técnicos inesperados. Usarlo por error en un mensaje funcional hace que el usuario vea un error genérico sin explicación.
:::

::: tip BUENA PRÁCTICA — regla rápida de decisión

- El usuario tiene que actuar (campo vacío, ID no existe, código duplicado) → `'# mensaje #'`
- El error es inesperado y debe investigarse en logs → sin `#...#`
...
### 8. Para LOS QUE TOCAN ESQUEMAS EN PRODUCCIÓN — desplegar una nueva edición sin parada · `OPCIONAL` {#practica-ebr}

::: info SOLO PARA LOS QUE TOCAN ESQUEMAS EN PRODUCCIÓN (normalmente ANALISTAS)
...
- [Ejercicio 2A — Diseño de vistas](../ejercicio-2a-tablas-vistas/): diseñar `VRES_FRANJA_HORARIO` y `VRES_HORARIO_DIA`, decidir alias, JOIN, filtros y compararlas con las vistas reales del schema.
- [Ejercicio 2B — Procedimientos en paquetes](../ejercicio-2b-paquetes/): implementar `ACTUALIZAR_BLOQUEADO`, `CREAR_HORARIO_DIA` y `CREAR_RESERVA` con validaciones reutilizables (`VALIDAR_ID_POSITIVO`, `VALIDAR_FLAG`, `VALIDAR_FECHAS`) y detección de solapamiento.
...
Antes de pasar a .NET, completa los dos ejercicios entregables ([2A — vistas](../ejercicio-2a-tablas-vistas/) y [2B — paquetes](../ejercicio-2b-paquetes/)) para practicar tanto el contrato de lectura como la escritura PL/SQL. En la parte .NET veremos cómo conectar `PKG_RES_TIPO_RECURSO` desde `ClaseOracleBD3`, escribir DTOs como `ClaseTipoRecurso` y exponer el CRUD como API REST leyendo `P_CODIGO_ERROR` / `P_MENSAJE_ERROR`.
...
Un `REF CURSOR` **no es el resultado** de la consulta: es un _handle_ (puntero) a un conjunto de filas que Oracle aún no ha materializado. Conviene conocer cómo lo gestiona el motor para usarlo bien y entender sus garantías.
...
| `OPEN ... FOR`                 | Oracle parsea la consulta (o reutiliza el plan de la _shared pool_), liga las bind variables y **fija el SCN de lectura**. No materializa filas. | Una vuelta de red, _soft parse_ si la consulta ya estaba en cache. |
...
La tabla aparece **una sola vez**: las tablas no son edicionables. Cambiar su estructura física (añadir o renombrar columnas) sin downtime requiere _editioning views_ y _crossedition triggers_, un escenario más avanzado fuera del alcance de esta sesión.
...
EXCEPTION
  WHEN DUP_VAL_ON_INDEX THEN
    -- Formato '# mensaje #': el extractor de .NET lo clasifica como EnumBDException.Usuario.
    -- Sin #...#, Oracle devolvería el mensaje técnico de la constraint, invisible al usuario.
...
| ----------------------------------------------------------------------------------------------------------------------- | ----------------------------- | --------------------------------------------------------------------------------------------------------- |
| [← Sesión 2: Ejercicio: inspección del schema y catálogo](../../../01-oracle/sesiones/sesion-02-ejercicio-fundamentos/) | [Índice del curso](../../../) | [Sesión 4: Ejercicio: diseño de vistas →](../../../01-oracle/sesiones/ejercicio-2a-tablas-vistas/) |
