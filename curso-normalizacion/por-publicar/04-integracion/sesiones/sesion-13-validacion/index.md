---
title: "Sesión 13: Validación en todas las capas"
...
## 13.1 El cambio profundo: de excepciones a `Result<T>` {#cambio-profundo}
...
### 13.1.1 El modelo anterior (que ya no usamos)
...
<!-- diagram id="s13-modelo-antiguo" caption: "Modelo anterior: cualquier problema es una excepción que el middleware captura" -->
...
### 13.1.2 El modelo actual: `Result<T>` + `HandleResult`
...
<!-- diagram id="s13-modelo-actual" caption: "Modelo actual: error esperable viaja en Result<T>; las excepciones siguen yendo al middleware solo para lo realmente inesperado" -->
...
### 13.1.3 ¿Qué ha cambiado de verdad?
...
### 13.1.4 El middleware sigue ahí, pero para menos cosas
...
- Loggear con Serilog (sesión 21).
- Enviar correo al equipo si el entorno está configurado para ello (sesión 14).
...
## 13.2 Las cuatro capas de validación {#pipeline}
...
<!-- diagram id="s13-pipeline-validacion" caption: "Pipeline completo: cuatro filtros sobre los datos del cliente, una sola respuesta" -->

### 13.2.1 Tabla resumen
...
### 13.2.2 El formato de respuesta es siempre el mismo
...
## 13.3 DataAnnotations: la primera línea {#data-annotations}
...
### 13.3.2 Lo que NO debe ir en DataAnnotations
...
Para todo lo anterior, ver §13.4.
...
## 13.4 FluentValidation: reglas cruzadas y asíncronas {#fluent-validation}

Cuando una regla no cabe en un atributo, FluentValidation expresa la lógica en código C# pleno.

### 13.4.1 Instalación y registro
...
### 13.4.3 Regla cruzada con error asignado a un campo
...
### 13.4.4 Regla cruzada sin campo claro (error global)
...
### 13.4.5 Regla asíncrona contra la BD
...
### 13.5.1 El formato del mensaje en el paquete
...
### 13.5.2 Lo que ve Vue es lo mismo, venga de donde venga
...
## 13.6 `RegistrarErrorTecnico`: el correo y el log de los errores que pasan {#registrar-error-tecnico}
...
### 13.6.1 Por qué este es el sitio del correo
...
flowchart LR
    ...
    RTE -->|ILogger Warning| LOG[Serilog · sesión 21]
...
<!-- diagram id="s13-registrar-error" caption: "Cada Failure pasa por RegistrarErrorTecnico antes de salir; aquí es donde enchufar log estructurado y correo" -->
...
### 13.6.2 ¿Y las excepciones que no son `Result`?
...
- **Sesión 14 — Errores**: ampliación de `ErrorHandlerMiddleware`, `ClaseErrores` (la utilidad UA para componer el mensaje del correo) y políticas de retry.
- **Sesión 21 — Serilog**: sinks Console / Oracle / File / Email. `RegistrarErrorTecnico` deja de loguear como `Warning` y emite eventos estructurados con propiedades (`Code`, `UserCodPer`, `RequestPath`).
:::

## 13.7 Vue: pintar errores con `useGestionFormularios` {#vue-gestion-formularios}
...
### 13.7.2 Patrón de submit canónico
...
<!-- diagram id="s13-adaptar-problem-details" caption: "adaptarProblemDetails reparte cada clave del JSON entre modelState y erroresGlobales" -->

### 13.7.3 Template: errores globales y por campo
...
### 13.7.5 `errorDeCampo` vs `erroresDeCampo`
...
### 13.7.6 Validación cliente con Zod: `validarConEsquema`
...
<!-- diagram id="s13-zod-vs-servidor" caption: "Zod y el servidor empujan al mismo modelState; el template no distingue origen" -->
...
## 13.8 Tabla resumen: de Oracle a Vue {#resumen}
...
## 13.9 Pruébalo en el proyecto {#sandbox}
...
- Esboza el validator `HorarioSemanalDtoValidator` con la regla cruzada de **solapamiento de franjas** (clave `""`). En la sesión 18 (Pinia) le añadirás el estado compartido.
...
En la **sesión 14 — Errores** entramos en el `ErrorHandlerMiddleware` de la plantilla y en `ClaseErrores`: cómo se construye el correo al equipo, qué política de retry tiene la cola, y dónde se filtran los errores que no merecen notificación (cancelaciones de cliente, requests interrumpidas, etc.). En la **sesión 21 — Serilog** sustituimos el `ILogger` que aparece en `RegistrarErrorTecnico` por un pipeline estructurado con sinks Console, Oracle, File y Email.
...
|---|---|---|
| [← Sesión 12: Llamadas a la API y autenticación](../../../04-integracion/sesiones/sesion-12-api-autenticacion/) | [Índice del curso](../../../) | [Sesión 14: Gestión de errores de extremo a extremo →](../../../04-integracion/sesiones/sesion-14-errores/) |
<!-- NAV:END -->
