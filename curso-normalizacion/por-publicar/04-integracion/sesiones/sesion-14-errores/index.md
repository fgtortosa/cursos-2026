---
title: "Sesión 14: Gestión de errores de extremo a extremo"
...
## 14.0 Las dos formas JSON del error: `ProblemDetails` y `ValidationProblemDetails` {#formas-json}
...
El resto de la sesión cubre **cómo** se construyen estas dos formas (§14.1 a §14.6) y **cómo** las recoge el cliente (§14.7 a §14.8).

## 14.1 El viaje de un error de extremo a extremo {#viaje-error}
...
<!-- diagram id="s14-viaje-error" caption: "Viaje completo de un error: tres caminos hacia el usuario y dos hacia el equipo" -->

### 14.1.1 Las tres trayectorias
...
### 14.1.2 Lo que se mantiene del modelo histórico UA
...
## 14.2 Anatomía del `Error` UA {#anatomia-error}
...
### 14.2.1 Para qué sirve cada campo
...
### 14.2.2 Por qué dos mensajes distintos
...
<!-- diagram id="s14-dos-mensajes" caption: "El Error transporta dos mensajes hacia destinos distintos: uno limpio al cliente, otro técnico al equipo" -->
...
### 14.2.3 Cómo construir un `Error` desde un servicio
...
Si la regla de negocio prevé que "el recurso puede no existir" o "el id puede llegar inválido", **no son excepciones**: son resultados posibles. Devuélvelos con `Result<T>.NotFound` / `Result<T>.Validation`. Las excepciones se reservan para lo imprevisto (BD caída, NRE, timeout) y las atrapa el middleware (§14.5).
:::

Los `params object?[] messageArgs` se guardan en `MessageArgs` y se aplican como `string.Format` cuando se localice el mensaje. Lo verás en §14.3 con los formatos Oracle.
...
Si no es `Success`, `Value` es `null` y hay un `Error`. `HandleResult` (§14.2.4) se encarga del resto — tú no tienes que mirar el `Error` a mano.
:::

### 14.2.4 `HandleResult` / `HandleCreated` / `HandleNoContent`: los traductores únicos `Result<T>` → HTTP {#handleresult}
...
private ActionResult HandleError(Error error)
{
    ...
    var mensaje = LocalizarMensaje(error);                    // §14.3
    ...
    RegistrarErrorTecnico(error, mensaje);                    // §14.6 ruta B
...
### 14.2.5 Cómo se escribe un endpoint del curso {#endpoint-canonico}
...
## 14.3 Los cuatro formatos de mensaje desde Oracle {#formatos-oracle}
...
### 14.3.1 Caso 1 — Error técnico (no visible)
...
### 14.3.2 Caso 2 — Literal visible al usuario
...
### 14.3.3 Caso 3 — Localizado con `Resources.X.Y` (con o sin parámetros)
...
### 14.3.4 Caso 4 — Externo UXXI (`PKG_X#COD#FALLBACK#args`)
...
### 14.3.5 Reglas de fallback resumidas
...
### 14.3.6 Buenas prácticas de PL/SQL
...
## 14.4 Las cuatro excepciones UA y cuándo siguen teniendo sentido {#excepciones-ua}
...
<!-- diagram id="s14-jerarquia-excepciones" caption: "Las cuatro excepciones UA históricas y su namespace/uso" -->

### 14.4.2 Cuándo usar cada una
...
### 14.4.3 El patrón `try { } catch (BDException ex) { … }` vive en `EjecutarPaqueteAsync`
...
## 14.5 `ErrorHandlerMiddleware`: el último cortafuegos {#middleware}
...
<!-- diagram id="s14-middleware-decision" caption: "ErrorHandlerMiddleware: clasifica la excepción, decide notificar, y responde según sea API o MVC" -->

### 14.5.2 Lo que decide la notificación
...
### 14.5.4 `UseExceptionHandler("/Error")` que ya tiene el proyecto
...
La sesión 13 te enseña los dos modelos para que sepas cuándo merece la pena cablear el middleware UA (lo veremos en §14.6).
:::

## 14.6 `AddClaseErrores`: notificación por correo {#add-clase-errores}
...
### 14.6.1 Configuración mínima en `appsettings.json`
...
### 14.6.4 Los dos puntos de enganche del correo
...
<!-- diagram id="s14-dos-rutas-correo" caption: "Dos puntos de inyección del correo: middleware para lo que escapa, RegistrarErrorTecnico para lo controlado pero relevante" -->
...
### 14.6.5 Modo simple (v1.x) y compatibilidad
...
## 14.7 Notificación al usuario en Vue: la familia `useToast` {#toasts}

En el lado del cliente, los errores y avisos se presentan con la familia `useToast` de `@vueua/components`. Ya la viste en la sesión 10; aquí la repasamos como **vocabulario común** del bloque de integración.

### 14.7.0 El interceptor de `useAxios`: quién decide `.then` vs `.catch` {#interceptor-useaxios}
...
<!-- diagram id="s14-useaxios-interceptor" caption: "El interceptor reparte la promesa: 2xx → .then; 4xx/5xx → .catch con el ProblemDetails en err.response.data." -->
...
Mapa de qué hace Vue con cada status (lo aplicará el código de §14.7.5):
...
### 14.7.2 Matriz de decisión: ¿qué toast para qué situación?
...
<!-- diagram id="s14-matriz-toasts" caption: "Qué variante de useToast usar según el evento" -->
...
### 14.7.3 Toast de operación larga (patrón canónico)
...
### 14.7.4 Grupos: cerrar varios toasts a la vez
...
### 14.7.5 Patrón canónico de formulario: `useGestionFormularios` + `peticion<T>` {#patron-formulario}
...
## 14.8 Confirmar antes de operaciones destructivas {#confirmar}

Borrar, archivar, enviar definitivo: cualquier acción que el usuario no pueda deshacer **debe** pedir confirmación. `PopUpModal` de la sesión 11 ofrece las dos APIs (declarativa e imperativa) que ya conoces; aquí las repasamos en el contexto de errores.

### 14.8.1 Patrón canónico: confirmar → operar → toast
...
<!-- diagram id="s14-confirmar-borrar" caption: "Patrón canónico de confirmación antes de borrar" -->

### 14.8.2 Implementación con la API imperativa
...
### 14.8.3 Cuándo usar la API declarativa
...
### 14.8.4 Buenas prácticas de UX en confirmaciones
...
## 14.9 Sandbox y resumen final {#sandbox}

### 14.9.1 Casos que puedes disparar hoy

| Caso | Cómo dispararlo | Qué cubre de esta sesión |
|------|-----------------|---------------------------|
| Error 404 controlado | Probador → botón `recurso-no-existe` (ORA-20702) | §14.1, §14.2, §14.3 (caso 3 con args), `Result.NotFound` |
| Error 400 controlado | Probador → botón `recurso-con-asociados` (ORA-20703) | §14.1, §14.3 (caso 3), banner global en `useGestionFormularios` |
| Error 500 controlado con `TechnicalMessage` | Probador → botón `error-tecnico` | §14.1, §14.6 (la ruta B del correo desde `RegistrarErrorTecnico`) |
| Error 400 automático por DataAnnotation | Scalar → POST `/api/TipoRecursos` con `Codigo=""` | §12.3 (DataAnnotations) y §14.1 (validación bloqueada) |
| Toast de éxito | Cualquier guardado correcto desde el CRUD de la sesión 11 | §14.7 — `avisar` |
| Confirmación + borrado | "Eliminar" del CRUD integrador (`Sesion11CrudRecursos.vue`) | §14.8 — `PopUpModal` |

### 14.9.2 De Oracle a Vue, todo junto
...
### 14.9.3 Checklist para una funcionalidad lista para producción
...
## 14.10 Tarea progresiva del proyecto final {#tarea-pf}
...
|---|---|---|
| [← Sesión 13: Validación en todas las capas](../../../04-integracion/sesiones/sesion-13-validacion/) | [Índice del curso](../../../) | [Sesión 15: DataTable de extremo a extremo →](../../../04-integracion/sesiones/sesion-15-datatable/) |
<!-- NAV:END -->
