---
title: "Sesión 9 — Tests y práctica IA"
description: Autoevaluación, preguntas, respuestas y práctica IA-fix para la Sesión 9 (Vue 3, TypeScript y primer componente).
outline: [2, 2]
search: false
---

# Sesión 9 — Tests y práctica IA

Material de evaluación de la [Sesión 9: Vue 3, TypeScript y tu primer componente](../../sesiones/sesion-09-vue-typescript-fundamentos/).

## Contenido

- [Autoevaluación rápida](./autoevaluacion.md) — 5 preguntas de 10 minutos para cerrar la clase.
- [Preguntas de test](./preguntas.md) — banco de 21 preguntas tipo test con cuatro opciones.
- [Respuestas](./respuestas.md) — solucionario razonado del banco anterior.
- [Práctica IA-fix](./practica-ia-fix.md) — componente con errores típicos para que la IA los corrija.
