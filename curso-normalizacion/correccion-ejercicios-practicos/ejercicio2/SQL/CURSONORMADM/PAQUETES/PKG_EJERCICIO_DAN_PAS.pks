CREATE OR REPLACE EDITIONABLE PACKAGE "CURSONORMADM"."PKG_EJERCICIO_DAN_PAS" AS
  TYPE T_CURSOR IS REF CURSOR;

  /* Paquete de gestión de franjas horarias y reservas por recurso.

     IMPORTANTE: Las operaciones validan pertenencia por ID_RECURSO para evitar
                 modificaciones cruzadas entre recursos.
     IMPORTANTE: Las operaciones de escritura aplican validaciones funcionales
                 internas (rangos horarios, solapamientos, coherencia temporal).

     11/05/2026  DPAS  Se implementan operaciones de actualización atómica.
     12/05/2026  DPAS  Se añade creación de horarios con validación de solapamiento.Se incorpora creación de reservas con control de solapamiento
...
END PKG_EJERCICIO_DAN_PAS;
/
CREATE OR REPLACE EDITIONABLE PACKAGE BODY "CURSONORMADM"."PKG_EJERCICIO_DAN_PAS" AS
  ...
  BEGIN
    ...
    IF P_DIA IS NULL OR P_DIA NOT BETWEEN 1 AND 7 THEN
      RAISE_APPLICATION_ERROR(-20820, 'El día debe estar entre 1 y 7.');
    END IF;

    IF P_HORA_INICIO NOT BETWEEN 0 AND 23 THEN
      RAISE_APPLICATION_ERROR(-20821, 'La hora de inicio debe estar entre 0 y 23.');
    END IF;

    IF P_HORA_FIN NOT BETWEEN 0 AND 23 THEN
      RAISE_APPLICATION_ERROR(-20822, 'La hora de fin debe estar entre 0 y 23.');
    END IF;
